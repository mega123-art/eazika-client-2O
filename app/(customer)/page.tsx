...
import { ShopService, Category, ShopProduct } from "@/services/shopService";
        ...
        <motion.section variants={itemVariants} className="relative z-20">
          <div className="relative">
            <div className="relative group">
              ...
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-hover:text-yellow-500 transition-colors pointer-events-none" size={20} />
              {searchQuery && (
                <button 
          ...
          <div className="relative">
            ...
            >
              {isLoading ? (
                 // Loading Skeletons
                 [1,2,3,4,5].map(i => (
                    <motion.div key={i} variants={itemVariants} className="flex flex-col items-center space-y-2 shrink-0">
                        ...
                        >
                          <div className="w-16 h-16 md:w-20 md:h-20 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-center group-hover:border-yellow-500 dark:group-hover:border-yellow-500 transition-colors duration-200 overflow-hidden relative">
                             {category.image ? (
                                 ...
                                 <Icon size={32} className="text-gray-400 dark:text-gray-500 group-hover:text-yellow-600 dark:group-hover:text-yellow-400 transition-colors" />
                             ) : (
                                 <div className="text-2xl font-bold text-gray-400">{category.name.charAt(0)}</div>
                             )}
                          </div>
        ...
        <motion.section variants={itemVariants}>
          ...
          >
            {isLoading ? (
                // Product Skeletons
                [1,2,3,4].map(i => (
                    <motion.div key={i} variants={itemVariants} className="bg-white dark:bg-gray-800 rounded-2xl h-64 animate-pulse border border-gray-100 dark:border-gray-700" />
                ))
            ) : (
                visibleProducts.map((item) => {
                  const liked = isClient ? isWishlisted(item.id.toString()) : false; 
                  return (
                      ...
                      >
                        ...
                        <div className="p-3 md:p-4 flex flex-col grow">
                            <div className="flex items-center space-x-1 mb-1">
                                ...
                                <span className="text-xs text-gray-500 dark:text-gray-400">{item.rating || '4.0'}</span>
                            </div>
